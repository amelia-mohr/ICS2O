...
var btnAddBag2 = document.getElementById('add-bag-2');
btnAddBag2.addEventListener('click', () => {
    let c = getCookie("cart");
    if (c == null) {
        // nothing in the cookie, just add our item
        document.cookie = "cart=" + JSON.stringify({Product2: num.innerHTML}) + "; path=/";
    } else {
        ...
        for (const [key, value] of Object.entries(obj)) {
            if (key == 'Product2') {
                ...
                obj['Product2'] = total.toString();
                found = true;
            };
        };
        if (!found) {
            obj['Product2'] = num.innerHTML;
        };
